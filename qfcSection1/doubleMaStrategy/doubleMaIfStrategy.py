...
class DoubleMaStrategy(CtaTemplate):
    ...
    def strategy(self, bar):
        ...
        exitExe = self.exitOrder(bar, exitSig)

        # 根据进场信号进场
        entrySig = self.entrySignal(envPeriod, signalPeriod)
        if exitExe==0:
            entryExit = self.entryOrder(bar, entrySig)

        # 触发止损
        if exitExe == 0 and entryExit==0:
            self.stoploss(bar)
    ...
    def exitSignal(self, signalPeriod):
        ...
        if arrayPrepared1:
            algorithm = maSignal()
            maCrossSignal, _, _ = algorithm.maCross(amSignal, self.paraDict)
